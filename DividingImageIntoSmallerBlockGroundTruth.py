...
pathFilename = "C:/Users/Julien/Desktop/Echantillon 1/GroundTruthAlign.png"
...
print("\nWe are creating the folder 'Original Ground Truth Image'. Please wait ...")

# Get the the pathname where all information about the sample is saved
pathToSave = os.path.split(pathFilename)[0]
newPathName = os.path.join(pathToSave, 'Original Ground Truth Image')

if os.path.exists(newPathName) == False:

    os.mkdir(newPathName)

print("The folder 'Original Ground Truth Image' is created.")
...
# Save Information
informationFileName = 'InformationFile3.pickle'

with open(os.path.join(pathToSave,informationFileName), 'wb') as f:
    pickle.dump([pathToSave, pathFilename, rootFilename], f)
